    ...
    if (this.state.checkedList.length) {
      Taro.chooseImage({
        ...
        success(img: any) {
          ...
          formData.append('file', img.tempFiles[0]);
          Taro.request({
            method: 'POST',
            url: 'http://api.mall.zishahui.net/img/upload-image',
            data: formData,
            header: {
              'Content-Type': 'multipart/form-data', // 这里试过type改为'application/x-www-form-urlencoded', false都一样
            },
          }).then(res => {
            console.log(res)
          })
